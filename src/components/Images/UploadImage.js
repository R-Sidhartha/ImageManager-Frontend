...
const UploadImage = ({ selectedFolderName, selectedFolder }) => {
  ...
  const { loadingUpload, errorUpload, uploadSuccess } = useSelector((state) => state.images);
  ...
  return (
    <form onSubmit={uploadImageHandler} className="bg-white p-4 rounded shadow-md mb-4">
      <h2 className="text-xl font-bold mb-2">Upload Image in <span className="text-green-700 font-bold text-xl uppercase">{selectedFolderName}</span></h2>
      <input
        type="text"
